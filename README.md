...
1. **Define Tasks**  
   ...
   - `IsComplete` (Optional): A function `func() bool`. If provided and returns `true`, the task's function will not be executed. The task is logged as skipped and considered successfully completed for prerequisite checking by dependent tasks.
